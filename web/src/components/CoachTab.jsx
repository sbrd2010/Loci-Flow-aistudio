...
export default function CoachTab({ payload, savePayload, saveSubPath }) {
  ...
  const apiKey = localStorage.getItem("loci_gemini_key") || "";
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <section className="card">
        ...
        <p style={{ fontSize: "12px", color: "var(--text-secondary)", marginBottom: "14px", lineHeight: "1.5" }}>
          On a tough day? Turn on Low Energy mode to see only easy P4 quick wins on the Today tab.
        </p>
        <div className="toggle-row">
          <span style={{ fontSize: "13px", fontWeight: "700", color: config.isLowEnergyMode ? "var(--warning)" : "var(--text-secondary)" }}>
            {config.isLowEnergyMode ? "⚡ LOW ENERGY ACTIVE" : "STANDARD FOCUS ACTIVE"}
          </span>
          <input type="checkbox" className="pill-toggle"
            checked={!!config.isLowEnergyMode}
            onChange={() => savePayload({ ...payload, config: { ...config, isLowEnergyMode: !config.isLowEnergyMode } })} />
        </div>
      </section>
