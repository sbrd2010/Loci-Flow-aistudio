...
// (Codex review finding).
// The trailing noun after a category word ("what job stuff do I have
// today?") isn't always literally "task(s)" — "stuff"/"things"/"items"/
...
// Every continuation phrase itself requires an end-of-clause right after it
// (not just a bare word boundary) — a bare "do i need" alone let arbitrary
// text follow, so "what health things do I need to know about pregnancy?"
// still slipped through; "do i need" now only counts if it's the end of the
// clause or immediately followed by a task verb (to do/handle/tackle/deal
// with/worry about) (Codex review finding). Also covers the "should I do/
// handle (first/next)" and "can I skip/ignore/put off" continuations
// "which <category> <noun>" questions naturally use, distinct from "what
// <category> <noun> do I have" phrasing — without them "which work items
// should I do first/next?" and "which work items can I ignore/put off?"
// stayed unguarded-noun-shaped but never matched at all, even though the
// unguarded "task(s)" form already accepted these same continuations
// (Codex review finding). Nouns are singular-friendly ("item"/"thing", not
// just "items"/"things") since "which work item should I do first?" is as
// natural as the plural form (Codex review finding).
// "now"/"right now" endings and "should i skip/ignore/put off" continuations
// mirror the immediate-time endings and skip-continuations the unguarded
// "task(s)" form already accepts — "which work items should I do right
// now?"/"which work items should I skip?" stayed light without them (Codex
...
const PRIORITY_FILTER_RE = new RegExp(
  ...
  // Bare list requests ("show me my errands", "list my errands", "what are
  // my errands?") mirror the "show me my <category> tasks" shape other
  // categories get from CATEGORY_TRAILING_NOUN_RE, which doesn't apply here
  // since "errands" has no separate trailing noun (Codex review finding).
  `\\b(?:show me|what are|list|check|tell me)\\s+my\\s+errands?\\b|` +
  // "of my" mirrors the optional "of my" every other "which <category>"
  // shape already accepts (Codex review finding). Singular "errand" is
  // accepted everywhere "errands" is (Codex review finding).
  `\\b(?:which|what)\\s+(?:of\\s+my\\s+)?errands?\\b(?=${CATEGORY_GENERIC_NOUN_END_RE}|\\s+${CATEGORY_GENERIC_NOUN_CONTINUATION_RE}${CATEGORY_GENERIC_NOUN_END_RE})`,
  "i"
);
...
const SINGLE_CATEGORY_PATTERNS = [
  ...
  // review finding). Bare list requests ("show me my errands") and singular
  // "errand" are covered too, mirroring PRIORITY_FILTER_RE's shapes (Codex
  // review finding).
  /\bwhat\s+(errands?)\s+do\s+i\s+have\b/i,
  /\b(?:show me|what are|list|check|tell me)\s+my\s+(errands?)\b/i,
...
// (Codex review finding).
// Also covers NEGATION_PRIORITY_RE's "bother with"/"skip"/"ignore"/"put
// off" verbs — those route to full_task fine on their own via "for
// <category>", but without a matching lead-in here, "what should I not
...
function extractCategoryLabels(clause) {
  ...
  while ((wordMatch = CATEGORY_WORD_RE.exec(clause))) {
    ...
    // "errands"/"errand" is a plain noun, unlike the adjective-like category
    // words ("work"/"job"/etc.), so it can appear as a compound-noun
    // modifier mid clause ("errands app design") without naming the
    // category being asked about — only counts when it ends the clause, is
    // followed by punctuation/end, or continues into a legitimate compound-
    // category list separator ("errands and health", "errands/health",
    // "errands, health") (Codex review finding). "/" and "&" don't require
    // a following space ("errands/health") the way "and" does, since the
    // rest of the category grammar accepts unspaced separators too (Codex
    // review finding).
    if (lower === "errands" || lower === "errand") {
      const rest = clause.slice(wordMatch.index + word.length);
      if (!/^\s*(?:[.?!,;]|$|and\s|[&/]\s?)/i.test(rest)) continue;
    }
    words.push(word);
  }
